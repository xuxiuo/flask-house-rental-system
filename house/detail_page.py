...
from models import House, User, Recommend, HouseRating
...
def detail(hid):
    ...
    avg_rating = db.session.query(func.avg(HouseRating.rating)).filter(
        HouseRating.house_id == hid).scalar()
    rating_count = HouseRating.query.filter(HouseRating.house_id == hid).count()
    user_rating = None
    if name:
        user = User.query.filter(User.name == name).first()
        rating_record = HouseRating.query.filter(
            HouseRating.user_id == user.id, HouseRating.house_id == hid).first()
        if rating_record:
            user_rating = rating_record.rating

    return render_template('detail_page.html', house=house, facilities=facilities_list,
                           recommend_li=recommend_li, avg_rating=avg_rating,
                           rating_count=rating_count, user_rating=user_rating)


@detail_page.route('/rate/<int:hid>', methods=['POST'])
def rate_house(hid):
    name = request.cookies.get('name')
    if not name:
        return jsonify({'valid': '0', 'msg': '请先登录后再评分'})
    user = User.query.filter(User.name == name).first()
    try:
        rating = float(request.form['rating'])
    except (KeyError, ValueError, TypeError):
        return jsonify({'valid': '0', 'msg': '请输入有效的数字评分'})
    if rating < 0 or rating > 10:
        return jsonify({'valid': '0', 'msg': '评分需在0~10之间'})
    existing = HouseRating.query.filter(
        HouseRating.user_id == user.id, HouseRating.house_id == hid).first()
    if existing:
        existing.rating = rating
    else:
        db.session.add(HouseRating(user_id=user.id, house_id=hid, rating=rating))
    db.session.commit()
    avg_rating = db.session.query(func.avg(HouseRating.rating)).filter(
        HouseRating.house_id == hid).scalar()
    rating_count = HouseRating.query.filter(HouseRating.house_id == hid).count()
    return jsonify({
        'valid': '1',
        'msg': '评分成功',
        'avg': round(float(avg_rating), 1) if avg_rating else 0,
        'count': rating_count
    })
