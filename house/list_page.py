...
def search_txt_info():
    ...
    if request.args.get('addr'):
        ...
        return render_template('search_list.html', house_list=result, search_type='addr', search_value=addr)
    ...
    if request.args.get('rooms'):
        ...
        return render_template('search_list.html', house_list=result, search_type='rooms', search_value=rooms_info)
...
@list_page.route('/list/price_house/<int:page>')
def return_price_list(page):
    house_num = House.query.count()
    total_num = math.ceil(house_num / 10)
    result = House.query.order_by(
        cast(House.price, Integer).asc()).paginate(page=page, per_page=10)
    return render_template('list.html', house_list=result.items, page_num=result.page, total_num=total_num,
                           list_type='price')
